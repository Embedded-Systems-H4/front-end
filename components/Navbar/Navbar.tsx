...
} from "@components/OutsideClickWrapper/OutsideClickWrapper";
  ...
  return (
    ...
    >
      <Flex
        ...
        {...rest}
      >
        {icon && <Icon mr="4" fontSize="16" w={"20px"} h={"20px"} as={icon} />}
        {children}
      </Flex>
    </Box>
  );
...
const MobileNav = ({ onOpen, ...rest }: MobileProps) => {
  ...
  const { isOpen, onToggle } = useOutsideClickContext();
      ...
      <HStack spacing={{ base: "0", md: "6" }}>
        <Flex alignItems={"center"}>
          <Menu>
            ...
            <MenuList bg={"gray.900"} borderColor={"gray.600"} p={1}>
              <MenuItem bg={"gray.900"} p={0} onClick={() => signOut()}>
                <chakra.span
                  p={2}
                  w={"100%"}
                  borderRadius={"md"}
